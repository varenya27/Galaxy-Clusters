...
def M_gas(r_c,n_c,beta,lim):
    ...
    def M(rx):
        n_r=n_c*((1+(rx/(r_c*kpc))**2))**(-3*beta/2)*constants.atomic_mass*0.6
        return n_r*4*np.pi*(rx)**2
    M_cur,err = quad(M,0,lim*mpc)
    return M_cur/M_sun*kg
...
def M_gas1(r_c,n_c,beta,lim):
    ...
    print(r_c,n_c,beta,lim)
    # r_c,err_r_c=R_c
    # n_c,err_n_c=N_c
    # beta,err_beta=Beta
    def M(rx):
        n_r=n_c*((1+(rx/(r_c*kpc))**2))**(-3*beta/2)*m*0.6
        return n_r*4*np.pi*(rx)**2
